...
- NEXT (rounds 24-25): `necessary_register_v1` rung 0 — a from-scratch hard-masked register transformer; rung 0 = INSTRUMENT CHECK (oracle codes replace the register embedding; generalization to unseen state-to-label permutations is the content); a constructed-substrate calibration ladder with a MANDATORY bridge (`register_bridge_v1`, rank-≤8 linear map from Qwen3-1.7B-Base source spans into the register coordinates) before any long-delay/moves/composition/effort/maps claim; no constructed-space headline before the bridge. Off: navigator, block-12 actuators, larger models as repair.
