...
1. **Round 34a first.** The preregistered matched-EDF core screen (Codex preregistration in `theory/EXPERIMENTS.md`, commit `f97a533`; tags `ctxcapA_raw` / `ctxcapB_raw` for the unresidualized `ctx_A`/`ctx_B` estimand, `ctxcapA_static` / `ctxcapB_static` separately for the `P_static`-residualized relation; no cross-estimand pooling) runs before any full Round 34 arm. Its analyzer implementation is being written by Codex; no run is authorized until it receives RUN-READY.
...
5. **Running chain (committed-analyzer copy, no inspection):** static-residualized completions `ctxS_A` / `ctxS_B`, then the parity check (`parity_head` / `parity_ref` A/B and a parity verdict). These are completions of the contextual-prefix comparator under the same audit #19 reading, not state-reading gate adjudications.

## Audit #20 correction — ctxB, ctxS_A, and the two Round 34a estimands
...
Round 34a is unrun and NOT-READY under Tier-1 re-review #3. The remaining reported blocker is exact reducer binding of candidate `state_match.rank`, `rank_tolerance`, and `retained_columns` to `selected_state`; do not launch until that invariant is closed. This is a localized instrument-integrity defect, not an experimental result.
