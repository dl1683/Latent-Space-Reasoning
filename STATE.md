...
- **Codex direction: predictive-state algebra (scratchpad/codex_escape_rn.txt).** Purely behavioral (no hooks/vectors). Test whether model supports product-of-registers overwrite algebra. Setters S_A^v, S_B^v; query Q_A, Q_B. Laws: idempotence, last-write-wins, disjoint-role commutation. ~5,040 CPU forwards. NEXT: implement `predictive_setter_algebra_v1`.
