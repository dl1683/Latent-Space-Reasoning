...
## Current statement (2026-09-01, PHASE 3 — co-designed dynamical carrier; Qwen micro-world closed; native math existence is axiomatic)

- **EAC-1: Endogenous Action Carrier v1 (2026-09-01, Phase 3). ENDOGENOUS_ACTION_CARRIER_PASS.** 150K CPU forwards (40K train + 8K eval + 2K integrity × 3 seeds), 62 seconds total. 359K-param model: structured world encoding (18 transitions as key-value pairs) + differentiable table lookup + context-dependent readout. 6 states, 3 actions, 120-token name pool. Trained on word lengths 1-3, tested on 1-4. **All 7 gates PASS across all 3 seeds:** held_out_accuracy (99.4-99.8%, ≥95%), self_patch (0.0, ≤1e-5), same_place (100%, ≥95%), same_place_jsd (100%, ≥90%), action_descent (95.6-100%, ≥95%), three_way_target (100%, ≥85%), three_way_exceed (eh=98.9-99.7%, ≥20%). Transplanting a donor carrier into a different host makes the host execute the donor's action on its own world. Two paths to the same state produce interchangeable carriers. Distance-from-claim: 0. **Next: held-out world transfer rung per Codex ruling.**

- **LAC-0: Learned Action Carrier (2026-09-01, Phase 3). SUPERSEDED by EAC-1.** Codex pivot postmortem ruling: program continues, frozen Qwen mining stops permanently. Original design: typed neural machine with 8-place navigation worlds, 4 primitive moves. EAC-1 implements the simpler preliminary experiment from the same ruling (6 states, 3 actions) and achieves full PASS. LAC-0's larger-scale design (8 places, composition, matched control) is the next rung.
