...
## Current statement (2026-08-30, RESTARTED in THEORY-FIRST mode; audits #27–#39 adopted; `docs/HANDOFF_2026_08_30.md` remains the orientation document; PSQ-2 v3 NO_INTERFACE 75.0%; exploratory d_4 in progress)

- **PSQ-2 empirical program (2026-08-30).** Two-dial world Z_8×Z_8, d_4 (H*=4, 341 words, 2 channels). PSQ-1 CLOSED: frozen Qwen3-{1.7B,8B}-Base and 8B-Instruct all NO_INTERFACE (cannot do modular arithmetic from few-shot). PSQ-2: LoRA fine-tuning Qwen3-1.7B-Base. v1 FAIL (60.2%, class imbalance). v2 FAIL (73.4% on 4-8 step OOD, composition doesn't generalize from 1-3 step training). v3 NO_INTERFACE (75.0% overall; 100% on 1-2 step, degrades to 47-63% on 6-8 step; model learns basic operations but can't compose deeply). Exploratory d_4 measurement in progress on v3 adapter (d_4 calibration: ≈0.43 at 75% accuracy, above 0.1 noise threshold; quasiconvexity test depends on rank ordering not absolute accuracy). Scientific question: does latent geometry respect behavior distances even with noisy model outputs?
