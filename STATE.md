...
- **Signature restatement v1 (2026-08-31).** O1 RESOLVED: S^G_g (representative-independent restatement from observable greedy signatures) EXISTS.
  ...
  - **Anti-echo alias control FAILS decisive gate (Phase 4c).** Five-arm test: decoy-only recovery 17-35% (decoy effective, Gate 1 passes); faithful alias 19-29% vs shuffled alias 21-25% — indistinguishable (Gate 3 fails, required ≥30pp). Direct R(g) after decoy recovers only 48%, so model is not pure last-mention — it integrates competing direct assignments with recency bias — but cannot resolve indirect alias references. Alias-only (no decoy) recovers 85-94%. S^G place preservation (100%) is consistent with R(g) being the only direct assignment, not semantic algebraic preservation. Non-naturality (v2, 70.8-89.6%) remains unexplained by pure textual echo.
