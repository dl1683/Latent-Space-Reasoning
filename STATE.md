...
- **Signature restatement v1 (2026-08-31).** O1 RESOLVED: S^G_g (representative-independent restatement from observable greedy signatures) EXISTS.
  ...
  - **Phase 4d terminal anti-echo factorial: PRE-REGISTERED, TOKENIZER PREFLIGHT PASS, UNRUN.** The primary estimand crosses direct `R(counterfactual) -> R(g)` against `R(g) -> R(counterfactual)` with identical complete token multisets; a recency pass demotes S^G regardless of alias outcomes. An explicit shared six-value domain plus a fixed-point-free involution removes out-of-type and accidentally faithful counterfactuals. Three arbitrary aliases are Latin-square counterbalanced over entity and position. The six alias cells are interpretable only if the alias-necessity arm moves answers away from the base signature. Gates and terminal rules are locked in `theory/PREDICTIVE_FIBER_ACTION_ALGEBRA.md`; implementation is isolated behind `python experiments/run_signature_restatement_v1.py --phase4d-only` and refuses to overwrite its future artifact. Tokenizer-only preflight passed all 12 single-token verbalizers, 96/96 static order-pair multiset checks, involution checks, and exact alias balance. No model forward pass or scientific result exists.
