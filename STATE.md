...
- `onewrite_recall_rung1` — staircase RUNG 1 (governance amendment 8; Codex round 19): LOCKED (`c71d44a`; ledger `onewrite_recall_rung1_lock`; config `experiments/config/onewrite_recall_rung1.json`; results `experiments/results/onewrite_recall_rung1/`) and RUNNING; no result yet. Same one-write machinery as v1 (frozen Qwen3-1.7B-Base, E/J interface, block-12 slot write, strict decoding, 8 tags, same-entity counterfactual donors) with every later difficulty removed: 24 TRAINING entities, training wording `TAG OF {NAME}:`, zero intervening tokens; 48 cases per arm per seed; arms write/cue/zero-hook/wrong/random/visible; gates unchanged from v1. Pre-lock validation PASS: visible 1.0, cue 0.125 (chance), visible completion 1.0, cue completion 0.96, zero-hook = cue row-for-row. If rung 1 fails, this interface stops without testing later rungs; if it passes, the next rung changes exactly one difficulty.
