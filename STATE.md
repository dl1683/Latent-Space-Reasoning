...
- **THEORY (restart 2026-08-30): foundation proposed; audit-#41 edits APPLIED in round 35 (f4069ea; repair pass 2 of 3); pending audit #42 (last permitted repair audit).** Theorem 3 candidate (round 35, not yet entered): with accessible responses = the action closure {o∘T_w} of an architecturally fixed base output, finite Z gives d_h = d_∞ for h ≥ |Z|²−1, and no uniform affine rate exists (T(x) = 2x, o(x) = δ_x, bounded Wasserstein: sup(d_∞ − d_h) = 1 for every h). Nothing currently proved is genuinely new mathematics. Open Problem 7 is now precise: directed executable path distance δ_E with d̄ ≤ δ_E (uniform differentiation), d̄ ≤ δ_E ≤ L·d̄ under L-quasiconvexity, equality in the affine world with reversible straight-line germs, and the no-germ counterexample (δ_E = +∞ while d_∞ is finite: the local-to-global gap is reachability, not geometry). `theory/AXIOMS.md` now carries 'Foundation: future-response geometry' (rounds 31–33, verbatim dialogue in `theory/dialogue/004.md`): D1–D7 (raw presented transition world; denizen-accessible response laws — external decoders are instruments; future-response pseudometric d_∞; identity = {d_∞ = 0}; descended maps; local response seminorm), Theorem 1 (d_∞ is the least pseudometric dominating immediate discrepancy under which every legal move is nonexpansive; identity is an equivalence; moves descend — proved for deterministic moves and distributional responses, i.e. LM prompt worlds), Proposition 2 (linear observability seminorm; places = cosets of the unobservable subspace; chart boundary G = λP — proved in the linear case), Open Problem 7 (local-to-global map problem — conjectured). Attribution: bisimulation-metric and Kalman-observability mathematics; novelty not claimed. C1/C2 preregistered in `theory/EXPERIMENTS.md`; NO compute until audit #40 is adopted.
