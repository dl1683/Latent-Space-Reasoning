...
- **Fusion-fission series (2026-08-31, Phase 2).** Qwen3-0.6B, two independently stored facts (HESK color, VORN color), four worlds (2×2). Five experiments completed:
  ...
  - **Codex audit (adopted verbatim):** "In one binary prompt, centered between-world activation contrasts coexist with nearly identical uncentered cosine, while a whole-prefix transplant instrument produces layer-dependent categorical response patterns. Baseline and intervention defects prevent interpreting those patterns as intrinsic fusion-fission." Five instrument defects: 75% baseline fatal (mixed worlds fail retrieval), layer alignment bug (pre-block in post-block slot), "R^n blind" overclaimed (PCA is R^n and sees it), v1d full transplant trivially expected, attention weights ≠ computation. NEXT: repair the instrument (v2) before building anything on top — near-perfect baseline, causal transfer matrix K_l, layer alignment fix, template replication, then component mediation with synergy decomposition.
