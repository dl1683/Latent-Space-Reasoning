...
- **THEORY — LM prompt-world EXTENSION: audit-#43 edits APPLIED (round 38); `native_horizon_v1` does NOT run now (round-38 ruling: do the response-completeness mathematics first); lock-readiness audit #44 deferred until an artifact is worth locking.** Round 39 WRITTEN (Codex, with corrections; pending audit #44): Theorem 7 (asymmetry of finite access — finite access gives lower bounds; nonidentity has a singleton finite witness; identity is not finitely certifiable from tables, though invariants/bisimulations can certify it; finite-state abstraction sufficient not necessary; LM identity claims are 'refuted' or 'not refuted at registered access'), D9/Theorem 8 (surgeon worlds on a common phase-typed carrier; d^D_∞ ≤ d^S_∞; Q^S → Q^D surjection; denizen-reachable landing = d^D_∞(m x, T_w x) = 0), the native bridge definition (d^D_∞(m_s ι_ℓ(x), ι_ℓ(y_s)) = 0, finitely refutable, not finitely confirmable), and the licensed record reading. Next candidate after audit #44: a small-budget refutation-oriented native bridge artifact on one real model with its proximal positive control. `theory/AXIOMS.md` 'Extension: language-model prompt worlds': Theorem 4 (finite-memory APPEND-ONLY worlds have exact finite-horizon response distance, d_∞⁺ = d⁺_{N−1}, under registered suffix/sliding semantics; removes the depth tail, not the branch problem), the discrete-map boundary (D7 absent for prompt worlds; the native map is the finite-depth response tree), Conjecture 5 (one-query discrimination horizon in explicit-legend worlds), Definition D8 + Proposition 6 (span substitutions as interventions; quotient descent required separately; place-changing iff d_∞(x, sx) > 0, state-realizing iff d_∞(sx, y) = 0). `theory/EXPERIMENTS.md` preregisters `native_horizon_v1` (model's own next-token laws only; h ∈ {0,1,2}; delayed-route control; span interventions) and RETIRES C1/C2 unrun. No compute until audit #43 adopts.
