...
- **THEORY — round 40 DONE: audit-#44 typing/status edits applied; native bridge specification written in `theory/AXIOMS.md` (phase-typed carrier; proved unchanged denizen transitions, totality, finite lower bound B_{W₀,C₀}; excess-discrepancy target-fiber estimand; same-carrier paste-back = proved validity fixture; cross-prompt residual paste = conjectured proximal control; centroid write = bridge edit; one model, block 16, one position, 24 entities, 7 words, ≤ 2,016 evaluations). Pending audit #45; no compute; no lock.**
