...
## Current statement (2026-08-29; audit #29 wording governs `state_bus_v1r1`, audit #28 wording governs the rest)
...
- Frozen-residual-stream line (coordinate_v1/v2/v3, interchange_v1): **stopped as an allocation pivot, not as a scientific conclusion.** Verbatim (audit #28): "Under the current apparatus budget, this sequence of frozen residual-stream constructions is not yielding a native-mathematics artifact, so open-ended layer, task, and decision-rule repair stops here. This is an allocation pivot, not evidence that frozen pretrained residual streams lack usable native structure."
...
- Next: reachability / control cost (direction round 8 rank 2, "run after interchange regardless of its result"; design in direction round 9, pending). Live alternative framings (audit #28 §3, verbatim in NOTEBOOK): predictive dynamics/flow; reachability and control cost; response-law topology / concept lattice; distributed transformation operators. Nothing above reopens.
...
## Real-model line 2026-08-29 (frozen residual stream stopped as an allocation pivot; `state_bus_v1r1` closed; `interchange_v2` audit pending)

Six real-model constructions ran on 2026-08-29 under the governance amendment (`AGENTS.md`, 2026-08-29): four frozen-residual constructions, one co-developed interface (`state_bus_v1r1`) and one bias-controlled frozen interchange (`interchange_v2`). Codex direction rounds 1–8 (`.codex_direction_r1..r8`, not committed; round 7 ledger `direction_r7_ruling`, `b9e4581`; round 8 ledger `direction_r8`, `97abf81`) chose and adjudicated them; audits #27 (ledger `coordinate_v3_audit27`), #28 (ledger `audit28`) and #29 (ledger `state_bus_v1r1_audit29`) fired once each; audit #30 on `interchange_v2` is pending. Only `coordinate_v3`, `state_bus_v1r1` and `interchange_v2` reached a causal intervention. Runner `experiments/run_coordinate.py` (configs `experiments/config/coordinate_v1.json`, `coordinate_v2.json`, `coordinate_v3.json`); runner `experiments/run_interchange.py` (`experiments/config/interchange_v1.json`, `interchange_v2.json`); runner `experiments/run_state_bus.py` (`experiments/config/state_bus_v1.json`); shared helper `experiments/substitution_probe.py`. Each subsection carries its licensed sentence verbatim; the Current statement governs where wording differs.
...
Audit #28 diagnostic (not a rescue): the native signatures separate cat from dog on all three probes relative to a calibration midpoint (24/24); raw sign was locked but was a poor design (fixed verbalizer prior). Cheapest moot-maker (audit #28): a fresh bias-controlled `interchange_v2` with calibration-midpoint/paired contrasts preregistered, an on-manifold wrong-donor control, and clustered reporting — authorized by direction round 8 and run as `interchange_v2` (subsection below; pre-declared FAIL, audit #30 pending).
...
### interchange_v2 — bias-controlled operational interchangeability, Qwen3-1.7B-Base block 12: pre-declared FAIL; audit #30 pending (wording pending)
...
Audit #29, without inspecting this run, flagged the lock as not confirmatory-run-ready (third-donor identities not counterbalanced; gates pooled across cat and dog; preregistered short decodes not implemented); audit #30 adjudicates these and the result. Until it lands, the only licensed statement is the pre-declared status above.
