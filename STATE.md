...
- **Signature restatement v1 (2026-08-31).** O1 RESOLVED: S^G_g (representative-independent restatement from observable greedy signatures) EXISTS.
  ...
  - **Phase 4d terminal anti-echo factorial: NO_INTERFACE_OR_INVALID — TERMINAL DEMOTION** (`results/signature_restatement_v1/phase4d_results.json`). 2304 forward passes, ~110 min CPU. All integrity checks passed (100% domain validity, token multisets verified, alias counterbalancing exact). **Gate 1 (interface) FAIL:** D1 (confirming record) follow 97.9%/91.7% — model reads records matching its base belief. D2 (contradicting record) follow 43.8%/16.7% — far below 60% threshold; record-append confirms but cannot contradict. **Gate 2 (direct recency) FAIL:** logit order effect -0.043/-0.086, CI crosses zero — no recency bias. **Gate 3 (alias necessity) FAIL:** A1 counterfactual follow 4.2%/6.9% — aliases barely steer toward counterfactual. **Gate 4 (alias anti-echo) FAIL:** discordant-alias greedy follow too low despite strong logit effects (3.6/2.4 nats, tight CIs). **Terminal adjudication:** S^G terminally locked as literal append operator. No semantic canonicalizer upgrade licensed. The counterfactual interface lacks sufficient steering power to make the anti-echo question answerable.
