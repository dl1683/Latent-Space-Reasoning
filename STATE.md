...
- `onewrite_recall_v1` — BUILT (runner `experiments/run_onewrite_recall.py`, config `experiments/config/onewrite_recall_v1.json`; round-16 amendments applied); **pre-lock validation referred; not locked, not run.** Round-16 gate validation (ledger `onewrite_recall_v1_validation_r16`; `experiments/results/onewrite_recall_v1/validate_result.json`; no state involved): visible-copy accuracy 1.0 (A 1.0, B 1.0), visible completion 1.0; cue accuracy 0.0; visible − cue = 1.0; cue completion 0.031 (without the fact the model emits a number, not a listed tag), which fails the letter of the round-16 `cue completion >= 0.95` criterion. Referred to Codex round 17 for a ruling (kill by the letter, or amend the protocol criterion pre-lock); nothing is licensed until it lands.
- `necessity_navigator_v1` — BUILT and smoke-tested only (runner `experiments/run_necessity_navigator.py`, config `experiments/config/necessity_navigator_v1.json`; ledger `direction_r11_navigator_design`, `direction_r12_ruling`); an OPTIONAL one-round calibration control, UNRUN. Smoke (code-path validation, not a result): 2000 steps reach held-out top-1 0.879 vs historyless control 0.484; readouts execute (moves R 0.24 vs untrained 0.38, composition order 0.51, inverse ratio 0.63, distance Spearman 0.30). Runs once only if a clean real-model negative leaves the question open (round 15).
