...
- NEXT: `reachability_v1` (round-22 nulled reachable-response geometry measurement of the closed slot; locked `reachability_v1_lock`; running) → its one audit → the write-up of the structured negative (round 22: A once, then D). Rung 0b, alternate positions, larger models, the navigator and any block-12 actuator stay off.
