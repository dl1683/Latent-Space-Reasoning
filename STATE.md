...
## Current statement (2026-08-30, early; audits #27–#33 govern their constructions; audit #34 on rung 0 pending; Codex direction rounds 10/12/15/18/19/20 and governance amendment 8 govern allocation)
...
- NEXT: repaired `register_bridge_preflight_v1` (audit #38 repairs: entity × state crossing, paired destroyed-context arm, balanced within-fold label-shuffle null, dual ridge, tokenization identity, single bootstrap draw, explicit CPU/fp32 + wall clock, recomputable evidence) → smoke → lock → one run → one audit. Decision rule (audit #38): PASS → dialogue + held-out causal bridge test (map Qwen residuals to the writer centroids, inject into the frozen consumer); PARTIAL → lexical/context engineering evidence, no staircase advance; FAIL → close the synthetic register line as nonbridging under this route. The synthetic staircase (short delay, held-out constructed entities, unseen wording, long delay, moves, algebra, effort, maps), larger models, the navigator and any block-12 actuator stay off.
