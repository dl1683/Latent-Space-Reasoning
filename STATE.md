...
- **Current artifact: `coordinate_v1`** — a two-bit causal coordinate (tense × polarity) in the Qwen3-0.6B residual stream, `experiments/run_coordinate.py` + `experiments/config/coordinate_v1.json` (registered in NOTEBOOK re-contextualization #27; Codex direction dialogue rounds 1–4). **Status: demo stage stopped at calibration** — no block cleared the coefficient-one final-prompt-token calibration rule (`0/16` for both leave-one-family-out axes at every block, termination `1.0`; `experiments/results/coordinate_v1/demo.log`, `result.json`), so no held-out transport was run; independently, the explicit-instruction baseline was found **invalid for polarity on Qwen3-0.6B** (states `01` and `11` at `0/8` under the original wording; `01` still `0/8` under the wording repair), so tense × polarity was not a valid two-bit task on this model. Codex direction round 4 proposes the headline `UNINTERPRETABLE — INVALID POLARITY BASELINE` and a model/axis change; **the direction decision is pending with Codex** and no successor run is authorized. Nothing here is a claim about residual-stream coordinates generally.
