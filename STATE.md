...
- `oracle_actuator_rung0` — staircase rung 0 (Codex round 20 design; runner `experiments/run_oracle_actuator.py`, config `experiments/config/oracle_actuator_rung0.json`; results `experiments/results/oracle_actuator_rung0/run_result.json`; ledger `direction_r20_rung0_design`, `oracle_actuator_rung0_preflight`, `oracle_actuator_rung0_lock`, `oracle_actuator_rung0_seed11`, `oracle_actuator_rung0_result`, `oracle_actuator_rung0_diag`): RUN 2026-08-30 — **REGISTERED FAIL — `FAIL — ORACLE ACTUATOR/SITE/RETRIEVAL CONSTRUCTION`, unanimous (seeds 11/23/37); audit #34 pending — nothing beyond the ledger row is licensed.** Construction: frozen Qwen3-1.7B-Base, block 12, final-token `Internal record:` slot; no encoder; fixed immutable hashed codebook of eight unit centred-simplex codes (hash `3f2381fe…`; Gram off-diagonal −1/7); only a zero-init biasless J (16→2048) trained 400 balanced entity×code steps, lr 3e-3; 0.25×slot-norm cap in primary with full telemetry; uncapped replay diagnostic; 24 entities × 8 codes + cue + zero-hook + 8 fixed off-code random vectors per seed. Preflight (eight zero-J rows) PASS; locked before any nonzero-write behaviour was inspected. Numbers (ledger `oracle_actuator_rung0_result`): capped passes 0/3, uncapped passes 0/3; cap never active in any seed (pre-cap ‖Jc‖ ≤ 14 vs threshold 43, so capped and uncapped evaluations are identical); code-follow 0.219/0.245/0.245; per-code follow code 0 0.96/0.96/0.96, code 6 0.75/0.96/0.96, codes 1–5 0.0, code 7 0.04 in every seed; own-code 0.25 in every seed; wrong-code follow 0.21/0.24/0.24 (uplift 0.10/0.125/0.125 over the cue matching rate); cue true-tag 0.125 (the cue decode is the base prior `fask`, code 0's tag); random-vector true-tag 0.13/0.13/0.125; completion 0.96 in code/random/cue arms; zero-hook = cue row-for-row; training loss flat 1.5–1.75; 2903 s wall; no repair run. Read-only diagnostic on the saved J's (ledger `oracle_actuator_rung0_diag`; `diag_logit_lens.log`): the eight write vectors are near-orthogonal (mean pairwise cosine −0.14) with norms 12–23; own-tag first-token logit-lens rank ~10^5 of 151k in all codes and seeds; late-training loss by code: code 0 1.20–1.22, code 6 1.41–1.56, all others 1.59–2.10; every tag is two tokens with distinct first tokens.
...
- NEXT (2026-08-30, early): the rung-0 result is in (registered FAIL above). Audit #34 on `oracle_actuator_rung0` is running (fresh, unprimed); its wording replaces the rung-0 bullet's numbers-only record when adopted. Then Codex direction round 21 (rung 0b, a different actuator/site construction, or stop of this line — per the round-20 status rule and amendment 8). Navigator deferred. Ratio 3:1 (warning stands). No run before the audit and round 21.
