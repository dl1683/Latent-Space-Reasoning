...
- **THEORY (restart 2026-08-30): foundation proposed, audit #40 corrections being applied (round 34), pending audit #41.** `theory/AXIOMS.md` now carries 'Foundation: future-response geometry' (rounds 31–33, verbatim dialogue in `theory/dialogue/004.md`): D1–D7 (raw presented transition world; denizen-accessible response laws — external decoders are instruments; future-response pseudometric d_∞; identity = {d_∞ = 0}; descended maps; local response seminorm), Theorem 1 (d_∞ is the least pseudometric dominating immediate discrepancy under which every legal move is nonexpansive; identity is an equivalence; moves descend — proved for deterministic moves and distributional responses, i.e. LM prompt worlds), Proposition 2 (linear observability seminorm; places = cosets of the unobservable subspace; chart boundary G = λP — proved in the linear case), Open Problem 7 (local-to-global map problem — conjectured). Attribution: bisimulation-metric and Kalman-observability mathematics; novelty not claimed. C1/C2 preregistered in `theory/EXPERIMENTS.md`; NO compute until audit #40 is adopted.
