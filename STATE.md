...
- **PSQ empirical program (2026-08-30).** Two-dial world Z_8×Z_8. PSQ-1 CLOSED (frozen models NO_INTERFACE). PSQ-2 CLOSED: v1 FAIL (60.2%), v2 FAIL (73.4%), v3 NO_INTERFACE (75.0%); Codex direction review identified metric mismatch (max √JS degenerate for binary oracle), tautological causal test, invalid quasiconvexity surrogate; hyperparameter staircase stopped. Exploratory d_4 measurement running on v3 adapter (DIAGNOSTIC ONLY). PSQ-3 design in final lock round: shared Procrustes action operators on calibration/held-out split, product metric d_{2,4}, 4-step causal staircase, staged PSQ-3A (task-only) / PSQ-3B (equivariance) arms. Feasibility constraint: ~40h GPU for full design, incompatible with sustained-GPU rule — needs resolution. Distance-0 artifact: the shared, held-out-generalizing action operator.
