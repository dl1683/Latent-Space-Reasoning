...
## Current statement (2026-08-30, RESTARTED in THEORY-FIRST mode; audits #27–#39 adopted; `docs/HANDOFF_2026_08_30.md` remains the orientation document; PSQ-2 CLOSED; PSQ-3 design in lock round)

- **PSQ empirical program (2026-08-30).** Two-dial world Z_8×Z_8. PSQ-1 CLOSED (frozen models NO_INTERFACE). PSQ-2 CLOSED: v1 FAIL (60.2%), v2 FAIL (73.4%), v3 NO_INTERFACE (75.0%). d_4 diagnostic on v3 adapter COMPLETE: rho(D_hidden, D_model)=0.505 at layer 18, rho(D_model, D_oracle)=0.579, quasiconvexity 4.6% — significant latent geometry present at 75% accuracy (DIAGNOSTIC, not scientific result). PSQ-3 design v6 FINAL (repair-round cap reached): 5 lock rounds, all REVISE; rounds 3-4-5 = 3-consecutive-REVISE stop condition; math core confirmed sound round 3, remaining blockers are operational (training-set construction, geometry CI clustering, verdict pooling, workload units, hardware placement); v6 adopts all corrections mechanically, no round 6. Shared Procrustes action operators on calibration/held-out split (floor-based 32/32), product metric d_{2,4}, 16-probe state-separating panel, 6-step causal staircase with paired superiority adjudication, transductive PCA (all 64 states), probabilistic d_panel with JS₂. Inference forward budget: 375,908 + 15,000 training steps (stable/cloud hardware; local pilot only). Distance-0 artifact: the shared, held-out-generalizing action operator. Model revision: ea980cb0a6c2ae4b936e82123acc929f1cec04c1. Runner BUILT and REVIEWED: ~1450 lines. Pilot PASSED (25.1 fwd/s, 4.0s local GPU). Codex correctness review found 16 issues; 9 fixes applied (crash: oracle_profiles dict→list, torch.set_grad_enabled scope, prompt space tokenization; verdict-changing: gate contamination fix via training_keys, NaN for invalid controls, replay logit-diff metric, cross-seed verdict tracker, inter-seed model cleanup, dir() guards). 5 deferred operational items (geometry bootstrap uncalled, frozen-base partial, no checkpoint-resume, composition fixed-point conservative, training-set ordering). NEXT: full run on stable/cloud hardware.
