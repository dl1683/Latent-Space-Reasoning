...
- **Fusion-fission series (2026-08-31, Phase 2).** Qwen3-0.6B, two independently stored facts (HESK color, VORN color), four worlds (2×2). Five experiments completed:
  ...
  - **Current instrument status:** Clean (zero self-patch). The transplant mechanism itself introduces no noise. Config-dependent causal transfer exists but the SEPARATE/FUSED binary doesn't emerge — all observed patterns are PARTIAL (both diagonal and off-diagonal effects present). NEXT: investigate config dependence, consider `typed_splice_editor_v2` direction from Codex synthesis.
