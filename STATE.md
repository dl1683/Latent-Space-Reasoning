...
- NEXT: `register_bridge_preflight_v1` locked (8beb8e9; rounds 27–28 + audit #38 repairs) and RUNNING → one audit (#39). Decision rule (audit #38): PASS → dialogue + held-out causal bridge test (map Qwen residuals to the writer centroids, inject into the frozen consumer); PARTIAL → lexical/context engineering evidence, no staircase advance; FAIL → close the synthetic register line as nonbridging under this route. The synthetic staircase, larger models, the navigator and any block-12 actuator stay off.
