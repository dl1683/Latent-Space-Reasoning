...
LLaDA tasks, with `48/48` eligible repair coverage. The label-free
complement-packet prompt artifact now contains `48` rows derived from fresh
source text and the predeclared expanded-aspect gap policy. The remaining v10
work is to generate packet rows with the frozen v9 policy and replay without
changing thresholds or realization rules after labels exist. If v10 passes, it
can become the clean transfer result for complement-first aggregation. If it
fails, the failure should become the next source-family boundary rather than a
prompt-tuning loop.
