...
## Program status (2026-08-30 — RESTARTED in THEORY-FIRST mode: math first, intuition second, compute third; audits #27–#42 adopted; no heavy LLM work; CPU freely, GPU only in small checkpointed bursts; orientation document `docs/HANDOFF_2026_08_30.md`)

- **Theory (restart 2026-08-30).** Adopted foundation: `theory/AXIOMS.md` 'Foundation: future-response geometry' (mathematics audit #42, after audits #40–#41 revisions; licensed wording in `STATE.md`). C1/C2 are preregistered in `theory/EXPERIMENTS.md` but NOT authorized (C1 blocked until its total 'other'-bin wrapper correction and a separate authorization; C2 unrun). The pause-time bullets below are retained as the closed record; their "program remains paused / on restart" language is historical.
