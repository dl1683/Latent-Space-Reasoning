...
- **signature_restatement_v1** (`b70aac2`; `results/signature_restatement_v1/results.json`). Tests O1: does representative-independent restatement S^G_g from observable greedy signatures exist? **O1 RESOLVED: YES.** Idempotence: 100% greedy (96/96), JSD 0.077/0.071. Descent: 12/12 (100%) / 15/15 (100%) — fixes S^W's one failure. Place preservation 100%. Non-naturality persists with S^G: JSD 0.193/0.189. **NEW FINDING:** Correction does NOT descend to G (58%/80%). Typed square is pointwise K(Cx) vs C(Kx), not quotient-level. Pattern: order-dependent — `_rev` representatives often ignore correction while `_std` accepts it. Correction effectiveness is itself presentation-path dependent.
