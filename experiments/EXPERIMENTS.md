...
## Program status (2026-08-30 — RESTARTED in THEORY-FIRST mode; audits #27–#48 adopted; PSQ-2 v3 NO_INTERFACE 75.0%; exploratory d_4 in progress; orientation document `docs/HANDOFF_2026_08_30.md`)

- **Theory (restart 2026-08-30).** Adopted foundation: `theory/AXIOMS.md` D1–D9, Theorems 1/4/7/8, Propositions 2/6, native bridge definition (mathematics audits #42 and #44; D7 audit #48 REVISE adopted; licensed wording in `STATE.md`). Codex should-continue review verdict: STOP current program form (central bet untested, ratio infinite with zero central artifacts, review loop negative-value); two paths pending user decision: (a) terminal salvage or (b) stop immediately. C1/C2 retired/not selected. The pause-time bullets below are retained as the closed record; their "program remains paused / on restart" language is historical.
...
## PSQ-2 — LoRA fine-tuning for modular state tracking + d_4 geometry measurement (2026-08-30; ACTIVE — exploratory d_4 in progress)
...
- **Exploratory d_4** (psq2_v3_d4): IN PROGRESS. d_4 measurement on v3 adapter at 75% accuracy. Calibration: d_4 ≈ 0.43 at this accuracy level (above 0.1 noise threshold). Tests quasiconvexity, Spearman correlation, and permutation null. Result caveated as sub-gate exploratory. Config: `psq2_v3_d4.json`.
