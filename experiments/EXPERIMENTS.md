...
- **signature_restatement Phase 4d terminal anti-echo factorial — PRE-REGISTERED, TOKENIZER PREFLIGHT PASS, UNRUN** (`experiments/run_signature_restatement_v1.py --phase4d-only`; future artifact `results/signature_restatement_v1/phase4d_results.json`). The primary test is the identical-token-multiset cross `R(counterfactual) -> R(g)` versus `R(g) -> R(counterfactual)`; final-block recency takes precedence over all alias outcomes. The shared value domain is explicit, the counterfactual is a fixed-point-free value involution, and three Latin-square maps counterbalance arbitrary aliases and assignment positions. A six-arm alias 2 x 2 includes a genuine necessity cell that must move answers away from the base signature. Gates, clustered estimands, terminal demotion rules, and the narrow pass sentence are locked in `theory/PREDICTIVE_FIBER_ACTION_ALGEBRA.md`. Tokenizer-only preflight passed: all 12 value verbalizers are single tokens, all 96 static crossed-order prompt pairs have identical complete token multisets, the involution is total/fixed-point-free/involutive, and alias balance is exact. No model forward pass or scientific result exists yet; any non-pass ends renderer tuning.
