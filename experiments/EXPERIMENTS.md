...
- **signature_restatement Phase 4d terminal anti-echo factorial — NO_INTERFACE_OR_INVALID, TERMINAL DEMOTION** (`experiments/run_signature_restatement_v1.py --phase4d-only`; `results/signature_restatement_v1/phase4d_results.json`). 2304 forward passes, ~110 min CPU. All integrity checks passed. **Gate 1 (interface) FAIL:** the model follows confirming records (D1: 97.9%/91.7%) but not contradicting ones (D2: 43.8%/16.7%, threshold 60%). **Gate 2 (direct recency) FAIL:** logit order effect negative (-0.043/-0.086), CI crosses zero — no recency bias in D3 vs D4 crossed-order arms. **Gate 3 (alias necessity) FAIL:** A1 counterfactual follow 4.2%/6.9% (threshold 60%). **Gate 4 (alias anti-echo) FAIL:** greedy rates too low despite strong logit effects (3.6/2.4 nats). S^G terminally locked as literal append operator. The counterfactual interface lacks steering power; anti-echo question unanswerable by this instrument.
