...
- **commitment_hysteresis_v1 — INCONCLUSIVE_ALLOCATION_STOP** (`4593ab0`; `results/commitment_hysteresis_v1/evidence.json`, `verdict.json`). 456 CPU forward passes (24 clean + 48×9 intervention). Tests whether prefix transplant at L21 leaves a causal trace that survives restoring the original host prefix after the commitment bottleneck (L25). Three families (ZOG/MIP, PLIM/KROT, HESK/VORN), 4 worlds each, 48 directed edges, 12 clusters. **Controls perfect**: self-patch 0.00, C25 (full restore) 0.00, all 12 clusters eligible. **M (move size): 0.48 [0.42, 0.54] PASS** — transplant substantially moves the output. **H (restored-prefix residue): 0.42 [0.37, 0.48] PASS** — perturbation survives host prefix restoration (genuine hysteresis). **T (donor-directed progress): 0.17 [0.12, 0.22] FAIL** — movement not specifically toward the donor's answer. **U (donor-residue after restoration): 0.11 [0.04, 0.16] FAIL** — surviving residue not donor-directed. **L (localization): 0.13 [0.08, 0.18]** — mild commitment localization but irrelevant since T and U fail. **Interpretation**: undirected hysteresis exists — the donor prefix imprints "something was different" on query/suffix positions through 4 layers of processing, and this persists after prefix restoration. But the imprint does not encode the donor's intended answer. T is highly variable across edges (range -0.28 to +0.52), suggesting some prefix configurations allow directed propagation while others do not.
