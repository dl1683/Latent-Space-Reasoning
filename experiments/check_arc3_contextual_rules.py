"""Check contextual ARC-3 rules against transition traces."""
...
class ContextualPredictionCheck:
    rule_id: str
    level_id: str
    step_index: int
    action: str
...
def _read_rules(path: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(payload, list):
        raise ValueError(f"Expected a JSON list of contextual rules: {path}")
...
def _matches_preconditions(trace: dict[str, Any], preconditions: dict[str, Any]) -> bool:
    before = trace.get("state_before") if isinstance(trace.get("state_before"), dict) else {}
    return all(_matches_expected_precondition(before.get(field), expected) for field, expected in preconditions.items())
...
def check_contextual_rules(
    traces: Iterable[dict[str, Any]],
    rules: Iterable[dict[str, Any]],
) -> list[ContextualPredictionCheck]:
    candidate_rules = [rule for rule in rules if rule.get("status") == "candidate"]
    checks: list[ContextualPredictionCheck] = []
    ...
    for trace in traces:
        ...
        for rule in candidate_rules:
            ...
            if not _matches_preconditions(trace, preconditions):
                continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("contextual_rules_json", type=Path)
...
def main() -> None:
    args = parse_args()
    checks = check_contextual_rules(_read_jsonl(args.trace_jsonl), _read_rules(args.contextual_rules_json))
