...
def build_facts(cfg):
    ...
    for i, f in enumerate([f for f in facts if f["split"] == "eval"]): f["cf_tag"] = T[(T.index(f["tag"]) + 1 + (i % (len(T) - 1))) % len(T)]   # balanced derangement, never the true tag
    return facts
...
def main():
    ...
    assert all(fl.startswith(cfg["slot"]) for fl in cfg["fillers"]); fl_len = [len(M.ids(fl)) for fl in cfg["fillers"]]; assert abs(fl_len[0] - fl_len[1]) <= 2 and min(fl_len) >= 64, fl_len
    ...
    for f in facts:                                                                            # cached source states; heldout facts also cache the counterfactual-tag sources
        ...
        if f["split"] == "eval": f["src_cf"] = [M.source_state(t.format(name=f["name"], tag=f["cf_tag"])) for t in tpl]
