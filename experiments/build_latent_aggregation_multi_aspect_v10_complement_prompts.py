"""Build the v10 complement-packet prompt artifact from label-free source rows.

This is the handoff between the v10 anchor/source run and complement-packet
generation. It deliberately avoids replay outputs, packet outputs, rubric hit
labels, and aggregation decisions. The prompt rows are derived from the frozen
task prompt, generated source text, stable trajectory ids, and the predeclared
expanded-aspect gap policy.
"""
...
DEFAULT_JSON_OUTPUT = Path(
    "eval_results/diffusion_language/latent_aggregation_multi_aspect_v10_complement_prompt_contract.json"
)
DEFAULT_REPORT_OUTPUT = Path(
    "docs/reports/diffusion/LATENT_AGGREGATION_MULTI_ASPECT_V10_COMPLEMENT_PROMPTS.md"
)

FORBIDDEN_LABEL_INPUTS = (
    "v10_complement_packet_raw",
    "v10_complement_packet_scores",
    "v10_complement_packet_replay",
    "v10_complement_packet_aspects",
    "v10_complement_packet_realized",
    "v10 replay labels",
    "v9 replay decisions",
    "rubric hit labels",
...
def build_complement_prompt_contract(
    *,
    ...
    if existing_packet_outputs and not allow_existing_packet_artifacts:
        ...
        raise ValueError(f"refusing v10 prompt build after packet/replay artifacts exist: {paths}")
    ...
    if empty_source:
        raise ValueError(f"v10 prompt rows have no source candidates: {', '.join(empty_source)}")
    ...
    manifest = {
        "schema": "latent_aggregation_multi_aspect_v10_complement_prompt_contract.v1",
        "generated_by": "experiments/build_latent_aggregation_multi_aspect_v10_complement_prompts.py",
        "freeze": {
...
def render_markdown(manifest: dict[str, object]) -> str:
    ...
    lines = [
        ...
        "",
        (
            "The v10 complement-packet prompt artifact is populated for the fresh 48-task "
            "transfer slice. It is not a result claim; it is the pre-packet generation "
            "contract that converts fresh source rows into source-supported complement prompts."
        ),
        "",
        "## Leakage Boundary",
        "",
...
def _validate_freeze(freeze: dict[str, object]) -> None:
    if freeze.get("schema") != "latent_aggregation_multi_aspect_v10_complement_freeze.v1":
        raise ValueError("v10 complement prompt builder requires the v10 complement freeze schema")
    transfer = _dict(freeze.get("transfer_contract"))
    if transfer.get("policy") != "v9_complement_packet_policy_fixed_before_v10_labels":
        raise ValueError("v10 complement prompt builder requires the frozen v9 packet policy")
    if not freeze.get("task_ids"):
        raise ValueError("v10 complement freeze has no task IDs")


def _validate_label_source(label_scores: dict[str, object], *, task_ids: list[str]) -> None:
    if not label_scores.get("run_id"):
        raise ValueError("v10 label source scores must include a run_id")
    if int(label_scores.get("all_generation_count", 0)) <= 0:
        raise ValueError("v10 label source scores must include generated rows")
    ...
    if missing:
        raise ValueError(f"v10 label source scores are missing frozen task IDs: {', '.join(missing)}")
...
def _label_free_rows(rows: list[dict[str, object]], *, task_ids: list[str]) -> list[dict[str, object]]:
    ...
    if missing:
        raise ValueError(f"v10 label raw source has no non-empty LLaDA rows for: {', '.join(missing)}")
    return filtered
...
def _prompt_row(*, task: dict[str, object], task_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if not task_rows:
        raise ValueError(f"no v10 label source rows for {task_id}")
