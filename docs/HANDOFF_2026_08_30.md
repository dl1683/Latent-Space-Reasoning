...
- Every session calls `bb_list` first (Iqidis blackboard MCP), records findings with provenance, and calls `bb_synthesis` before a verdict. If the blackboard tools are absent, say so and fall back to `STATE.md`/`NOTEBOOK.md`.
