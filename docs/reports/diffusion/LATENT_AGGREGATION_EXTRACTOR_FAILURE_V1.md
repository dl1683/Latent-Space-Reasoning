...
The frozen literal extractor is not mainly hallucinating components; it is missing paraphrased components. Lowering the literal threshold improves recall only modestly, so the next extractor should add paraphrase-aware matching rather than only retune the threshold.
