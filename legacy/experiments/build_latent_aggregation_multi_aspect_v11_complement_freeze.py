"""Build the v11 complement-packet transfer freeze.

V10 demonstrated that complement packets transfer to fresh planning tasks
(plan_393..plan_440) with frozen packet policy.  V11 continues this chain:
96 new tasks (plan_441..plan_536) from a dedicated v11 planning-task file,
LLaDA-only (no Dream candidates), and tighter statistical gates scaled for
the larger task set.
"""
...
DEFAULT_V11_TASKS = Path("experiments/latent_aggregation_v11_planning_tasks.jsonl")
DEFAULT_SCOUT_TASKS = Path("experiments/general_reasoning_tasks_scout.jsonl")
DEFAULT_V10_REPLAY = Path(
    "eval_results/diffusion_language/latent_aggregation_multi_aspect_v10_complement_packet_replay.json"
)
DEFAULT_V10_SOURCE_SCORES = Path(
    "eval_results/diffusion_language/latent_aggregation_multi_aspect_v10_complement_packet_scores.json"
)
DEFAULT_JSON_OUTPUT = Path("eval_results/diffusion_language/latent_aggregation_multi_aspect_v11_complement_freeze.json")
DEFAULT_REPORT_OUTPUT = Path("docs/reports/diffusion/LATENT_AGGREGATION_MULTI_ASPECT_V11_COMPLEMENT_FREEZE.md")
DEFAULT_LABEL_RAW = Path("eval_results/diffusion_language/latent_aggregation_multi_aspect_v11_raw.jsonl")
DEFAULT_LABEL_SCORES = Path("eval_results/diffusion_language/latent_aggregation_multi_aspect_v11_scores.json")
...
DEFAULT_PACKET_RAW = Path(
    "eval_results/diffusion_language/latent_aggregation_multi_aspect_v11_complement_packet_raw.jsonl"
)
DEFAULT_PACKET_SCORES = Path(
    "eval_results/diffusion_language/latent_aggregation_multi_aspect_v11_complement_packet_scores.json"
)
DEFAULT_PACKET_REPORT = Path(
    "docs/reports/diffusion/LATENT_AGGREGATION_MULTI_ASPECT_V11_COMPLEMENT_PACKET_REPORT.md"
)
DEFAULT_REPLAY_OUTPUT = Path(
    "eval_results/diffusion_language/latent_aggregation_multi_aspect_v11_complement_packet_replay.json"
)
DEFAULT_ASPECTS_OUTPUT = Path(
    "eval_results/diffusion_language/latent_aggregation_multi_aspect_v11_complement_packet_aspects.jsonl"
)
DEFAULT_REALIZED_OUTPUT = Path(
    "eval_results/diffusion_language/latent_aggregation_multi_aspect_v11_complement_packet_realized.jsonl"
)
DEFAULT_REPLAY_REPORT = Path(
    "docs/reports/diffusion/LATENT_AGGREGATION_MULTI_ASPECT_V11_COMPLEMENT_PACKET_REPLAY.md"
)

FROZEN_TASK_PRESET = "latent_aggregation_multi_aspect_v11_plan441_536"
FROZEN_TASK_IDS = tuple(f"plan_{index:03d}" for index in range(441, 537))
PRIOR_PLANNING_TASK_MAX = 440


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--tasks", type=Path, default=DEFAULT_V11_TASKS)
    parser.add_argument("--scout-tasks", type=Path, default=DEFAULT_SCOUT_TASKS)
    parser.add_argument("--v10-replay", type=Path, default=DEFAULT_V10_REPLAY)
    parser.add_argument("--v10-source-scores", type=Path, default=DEFAULT_V10_SOURCE_SCORES)
...
def main() -> int:
    ...
    manifest = build_freeze_manifest(
        ...
        v10_replay_path=args.v10_replay,
        v10_source_scores_path=args.v10_source_scores,
...
def build_freeze_manifest(
    *,
    ...
    if existing_outputs:
        ...
        raise ValueError(f"refusing v11 freeze after output artifacts exist: {paths}")
    ...
    v10_replay = json.loads(v10_replay_path.read_text(encoding="utf-8"))
    v10_source_scores = json.loads(v10_source_scores_path.read_text(encoding="utf-8"))
    v10_summary = _dict(v10_replay.get("summary"))
    v10_gate = _dict(v10_replay.get("gate_evaluation"))
    v10_boundary = _dict(v10_replay.get("evidence_boundary"))
    v10_source_summary = _dict(v10_source_scores.get("summary"))
    _validate_v10_boundary(v10_gate, v10_boundary, v10_summary)

    return {
        "schema": "latent_aggregation_multi_aspect_v11_complement_freeze.v1",
        "generated_by": "experiments/build_latent_aggregation_multi_aspect_v11_complement_freeze.py",
        ...
        "prior_evidence": {
            "boundary": (
                "V10 is the first fresh complement-packet transfer success: it passed all "
                "frozen numeric replay gates on held-out planning tasks (plan_393..plan_440) "
                "with complement-packet policy frozen before labels. V11 continues the chain "
                "with 96 new tasks (plan_441..plan_536), LLaDA-only, and tighter gates."
            ),
            "v10_replay": _diagnostic_ref(v10_replay_path, v10_summary),
            "v10_source_scores": _diagnostic_ref(v10_source_scores_path, v10_source_summary),
            "v10_evidence_boundary_status": v10_boundary.get("status"),
        },
        "freshness_contract": {
            "prior_planning_task_max": PRIOR_PLANNING_TASK_MAX,
            "rule": "all v11 planning IDs must be greater than every prior committed aggregation planning slice",
            "status": "passed",
            "forbidden_reuse": [
                ...
                "v10 task prompts (plan_393..plan_440)",
                "v10 labels or complement rows",
            ],
        },
        "task_mix_contract": {
            "purpose": "test complement-packet transfer on fresh LLaDA-only planning prompts at 2x scale (96 tasks vs 48)",
        ...
        "source_family_contract": {
            ...
            "packet_prompt_builder_required": (
                "Implement/run a v11 prompt builder after anchor/source rows exist and before packet generation. "
        ...
        "statistical_gates": {
            ...
            "minimum_complement_coverage_count": 72,
            ...
            "minimum_aggregate_win_count": 60,
        ...
        "v11_specific_gates": {
            ...
            "must_report_keyword_stuffing_audit": True,
        },
        "failure_taxonomy": [
...
def render_markdown(manifest: dict[str, object]) -> str:
    ...
    lines = [
        ...
        "",
        (
            "Freeze a fresh 96-task LLaDA-only transfer test continuing the v10 complement-packet "
            "policy. V10 passed all gates on plan_393..plan_440; v11 doubles the task count "
            "(plan_441..plan_536) and drops Dream candidates to isolate the LLaDA complement channel."
        ),
        "",
        "## Frozen Tasks",
        "",
        ...
        f"- V10 replay: `{_dict(prior.get('v10_replay')).get('path')}`",
        f"- V10 source scores: `{_dict(prior.get('v10_source_scores')).get('path')}`",
        f"- V10 evidence boundary: `{prior.get('v10_evidence_boundary_status')}`",
        "",
        "## Transfer Contract",
        "",
        ...
        *[f"- {item}" for item in transfer.get("required_sequence", [])],
        ...
        "## V11-Specific Gates",
        "",
        *[f"- `{name}`" for name, enabled in _dict(manifest.get("v11_specific_gates")).items() if enabled],
        "",
        "## Failure Taxonomy",
        "",
...
def _validate_v10_boundary(
    ...
    if gate_evaluation.get("overall_status") != "passed":
        raise ValueError("v11 freeze requires the committed passing v10 replay")
    if evidence_boundary.get("status") != "fresh_predeclared_complement_packet_transfer":
        raise ValueError("v11 freeze requires v10 to have fresh_predeclared_complement_packet_transfer status")
    if int(_float(summary.get("complement_coverage_count"))) < 36:
        raise ValueError("v11 freeze requires the committed v10 complement coverage result")
    if int(_float(summary.get("online_promoted_task_count"))) < 30:
        raise ValueError("v11 freeze requires the committed v10 online promotion result")
    if int(_float(summary.get("unsupported_addition_count"))) != 0:
        raise ValueError("v11 freeze requires clean v10 unsupported-addition safety")
    if int(_float(summary.get("hard_contradiction_count"))) != 0:
        raise ValueError("v11 freeze requires clean v10 contradiction safety")
...
def _assert_fresh_task_ids(task_ids: tuple[str, ...]) -> None:
    ...
    if stale:
        raise ValueError(f"v11 task ids must be fresh above plan_{PRIOR_PLANNING_TASK_MAX:03d}: {stale}")
...
def _label_command(label_raw_path: Path, label_scores_path: Path) -> str:
    return (
        ...
        "--report-output docs\\reports\\diffusion\\LATENT_AGGREGATION_MULTI_ASPECT_V11_LABEL_REPORT.md"
    )
...
def _replay_command(
    *,
    ...
    return (
        ...
        "--freeze eval_results\\diffusion_language\\latent_aggregation_multi_aspect_v11_complement_freeze.json "
...
def _task_theme_by_id() -> dict[str, str]:
    buckets = (
        ...
        "aspect_validation",
        "scale_robustness",
        "keyword_stuffing_resistance",
    )
    return {
        task_id: buckets[index // 12]
