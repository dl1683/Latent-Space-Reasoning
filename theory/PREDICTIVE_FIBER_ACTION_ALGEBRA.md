...
nAnti-echo alias control (Phase 4c) FAILS decisive gate. Five-arm test with nonce aliases (cedar→ZOG, amber→MIP, violet→PLIM): faithful alias recovery (19-29%) indistinguishable from shuffled alias (21-25%). Gate 3 (≥30pp separation) fails. Direct R(g) after decoy recovers only 48%, so model is not pure last-mention but integrates competing direct assignments with recency bias. Alias-only (no decoy) recovers 85-94%. $S^G$ place preservation (100%) is consistent with R(g) being the sole direct assignment — textual echo strengthened, not resolved.
