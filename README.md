...
| Hardware-dependent decoding determinism | **New finding (2026-08)** | Greedy decoding is not reproducible on a GH200/CUDA-13 stack: the same sequential call run twice differs on 2/5 tasks, and 8 byte-identical batch rows give 13/40 distinct completions (Igor Rivin, `experiments/greedy_trajectory_stability.json`). On an RTX 5090/CUDA-12.8 stack the same probe is bit-identical across three independent processes (1/8 distinct), while perturbed rows diverge (up to 7/8 distinct, 3 distinct answers). Embedding perturbation is a causal diversity source only on deterministic stacks; on non-deterministic stacks floating-point noise already produces the diversity. Any diversity claim must report the stack's noise floor. |
