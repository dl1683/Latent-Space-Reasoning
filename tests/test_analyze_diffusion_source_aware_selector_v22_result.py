...
from experiments.analyze_diffusion_source_aware_selector_v22_result import build_result_summary


def test_source_aware_selector_result_marks_utility_failure_when_overfiltered(tmp_path):
    ...
    assert result["summary"]["source_aware_selected_waste_count"] == 0
    assert result["summary"]["source_aware_selected_positive_count"] == 1
    assert result["decision"]["status"] == "precision_positive_utility_failed"
...
def _freeze():
    return {
        "target_surface": {
            ...
            "history_prefix_planning_delta_min": 0.2,
        }
    }
...
def _scores():
    return {
        "comparison_rows": [
            {
                "oracle_delta_vs_trajectory": 0.1,
...
def _targets():
    return {
        "rows": [
            {
                "candidate_lift_vs_trajectory": 0.10,
                "max_span_target_score": 2.1,
            ...
            },
            {
                ...
                "max_span_target_score": 2.1,
                "planning_quality_delta_vs_source": 0.12,
                "repair": "constraint_gap_span_phase_final_preserve_seeded_gated_repair",
                "task_id": "plan_b",
            },
        ],
